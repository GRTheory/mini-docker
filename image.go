...
func imageExistsByHash(imageShaHex string) (string, string) {
	idb := imagesDB{}
	parseImagesMetadata(&idb)
	for imgName, av1Images := range idb {
		for imgTag, imgHash := range av1Images {
...
func parseImagesMetadata(idb *imagesDB) {
	...
	if _, err := os.Stat(imagesDBPath); os.IsNotExist(err) {
		// If it doesn't exist crate an empty DB
...
func marshalImageMetadata(idb imagesDB) {
	...
	if err := os.WriteFile(imagesDBPath, fileBytes, 0644); err != nil {
		log.Fatalf("Unable to sve images DB: %v\n", err)
	}
}
...
func removeImageMetadata(imageShaHex string) {
	...
	if len(imgName) == 0 {
		log.Fatal("Could not get image details")
	}
...
func deleteImageByHash(imageShaHex string) {
	...
	if err != nil {
		log.Fatalf("Unable to get running contaienrs list: %v\n", err)
	}
	for _, container := range containers {
		if container.image == imgName+":"+imgTag {
			log.Fatalf("Cannot delete image because it is in use by: %s", container.containerId)
		}
	}
...
func downloadImageIfRequired(src string) string {
	...
	if downloadRequired, imageShaHex := imageExistByTag(imgName, tagName); !downloadRequired {
		...
		// Identify case where ubuntu:latest could be the same as ubuntu:20.04
