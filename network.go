...
func createIPAddress() string {
	byte1 := rand.Intn(254)
	byte2 := rand.Intn(254)
	return fmt.Sprintf("172.20.%d.%d", byte1, byte2)
}
...
// IPs which we will alse use for our containers.
...
// There seems to be a bug in the netlink library in that does not
